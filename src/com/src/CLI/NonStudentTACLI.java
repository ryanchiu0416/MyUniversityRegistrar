...
/*
    NonStudentTACLI implements CLIStrategy interface, and handles generating menus for non-Student TAs.
    Methods include presenting front-end menu for users and execute commands entered.

    Pattern: Strategy (concrete subclass)


    Note: The method implementation for NonStudentTACLI will look pretty much the same as FacultyCLI.
    This is because by definition, faculty and non-student TA are not the same role, although currently they
    have the same operations in this Registrar bounded context. For the purpose of development flexibility
    in the future, we want to create distinct classes for these two different roles.
*/
public class NonStudentTACLI implements CLIStrategy {
    private NonStudentTA user;
    private String name;
    public NonStudentTACLI(NonStudentTA f, String name) {
            this.user = f;
            this.name = name;
    }
    ...
    public void menu() throws IOException {
        ...
        while (!response.equals("Goodbye!")) {
            System.out.println();
            System.out.println(" *** Hi " + this.name + ". You're logged in to Registrar System as Non-student TA *** ");
